...
### Fixed
- **Auto-contrast header text color.** When a table had a custom
  `header_bg` (e.g., dark navy `#2c3e50`) but the preset's `header_text`
  was empty (e.g., `academic` preset), header text defaulted to black
  and was invisible on dark backgrounds. Now `HwpxBuilder.add_table`
  auto-computes a contrasting text color (near-white or near-black)
  via WCAG 2.0 relative luminance whenever `header_bg` is set but the
  preset doesn't override `header_text`.
